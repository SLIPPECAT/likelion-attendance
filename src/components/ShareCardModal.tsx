...
export const ShareCardModal: React.FC<ShareCardModalProps> = ({ summary, onClose }) => {
  ...
  const generateBlob = async (): Promise<Blob | null> => {
    ...
    return toBlob(cardRef.current, { pixelRatio: 3, backgroundColor: '#0d0d0d' });
  };
    ...
    >
      ...
      >
        <div className="flex items-center justify-between">
          <h3 className="font-bold text-gray-900 text-lg">출결 카드 공유</h3>
          <button
            type="button"
            onClick={onClose}
        ...
        <p className="text-xs text-gray-500 -mt-3">
          이름은 <strong className="text-gray-700">{maskedName}</strong> 처럼 일부만 표시돼요. 이미지는 브라우저 안에서만 생성되며 어디로도 전송되지 않습니다.
        </p>
        ...
        <div className="flex justify-center overflow-hidden rounded-2xl">
          <div
            ref={cardRef}
            className="w-[340px] shrink-0 bg-gradient-to-br from-[#1a1a1a] to-[#0a0a0a] rounded-[28px] p-7 text-white relative overflow-hidden"
          >
            <div className="absolute -top-16 -right-16 w-48 h-48 bg-[#ff6000]/25 rounded-full blur-3xl" />
            <div className="absolute -bottom-16 -left-16 w-48 h-48 bg-[#ff6000]/10 rounded-full blur-3xl" />

            <div className="relative z-10 space-y-6">
              {/* Header */}
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 rounded-xl bg-[#ff6000] flex items-center justify-center font-black text-sm">
                    사
                  </div>
                  <div>
                    <div className="text-xs font-bold leading-tight">멋쟁이사자부트캠프</div>
                    <div className="text-[10px] text-gray-400 leading-tight">출결 분석기</div>
                  </div>
                </div>
                <div className="text-[10px] text-gray-400 font-mono">{cohort}</div>
              </div>

              {/* Name & course */}
              <div>
                <div className="text-2xl font-extrabold">
                  {maskedName} <span className="text-sm font-normal text-gray-400">수강생</span>
                </div>
                <div className="text-xs text-gray-400 mt-1 line-clamp-2">{courseName}</div>
              ...
              {/* Stat row */}
              <div className="grid grid-cols-3 gap-2 text-center border-t border-white/10 pt-5">
                <div>
                  <div className="text-lg font-bold">{totalDays}일</div>
                  <div className="text-[10px] text-gray-400">총 교육일</div>
                </div>
                <div>
                  <div className="text-lg font-bold text-emerald-400">{pureAttendanceDays}일</div>
                  <div className="text-[10px] text-gray-400">순수 출석</div>
                </div>
                <div>
                  <div className="text-lg font-bold text-[#ff8a3d]">{totalPartialAbsences}회</div>
                  <div className="text-[10px] text-gray-400">지각·조퇴·외출</div>
                </div>
              </div>

              {/* Footer */}
              <div className="text-center text-[10px] text-gray-500 pt-3 border-t border-white/10">
        ...
        <div className="flex gap-2">
          <button
            ...
            disabled={isBusy}
            className="flex-1 flex items-center justify-center gap-1.5 bg-gray-100 hover:bg-gray-200 text-gray-800 font-semibold text-sm rounded-xl py-2.5 transition-colors disabled:opacity-50 cursor-pointer"
          >
